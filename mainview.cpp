...
{
    auto result = m_server->listen(QHostAddress::Any,PORT_NUMBER); // Host addr, port
    if (!result)
    {
        QMessageBox::critical( this,tr("EchoServer"),
...
{
    m_server->close();
    if ((m_socket != nullptr)&& m_socket->isOpen()) m_socket->close();
